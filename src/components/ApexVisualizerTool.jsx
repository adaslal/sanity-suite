...
// ─── Flowchart Viewer Panel (with insertion overlay) ────────────────────────
      ...
      <div className="flex items-center justify-between border-b border-border/50 px-4 py-2.5">
        ...
        {editable && svgContent && (
          <span className="text-[10px] text-muted/60 hidden sm:inline">
            Click nodes to rename &middot; Hover lines to insert
          </span>
        )}
      </div>
      ...
      <div className="relative flex-1 overflow-auto" style={{ minHeight: '400px' }}>
        {error ? (
          ...
          <div className="flex flex-col h-full">
            {/* Relative wrapper — SVG + insertion overlay share this coordinate space */}
            <div ref={wrapperRef} className="relative flex-1">
              <div
                ref={svgContainerRef}
                className={`flex justify-center p-6 [&_svg]:max-w-full [&_svg]:h-auto ${editable ? '[&_.node]:cursor-pointer [&_.node:hover]:opacity-80' : ''}`}
                dangerouslySetInnerHTML={{ __html: svgContent }}
              />

              {/* ── Insertion Point Overlays ── */}
              {insertionPoints.map((pt, i) => (
                <div
                  key={`ins-${i}`}
                ...
                >
                  <button
                    onClick={(e) => { e.stopPropagation(); onInsertAt && onInsertAt(pt.afterStepIndex); }}
                    className="h-6 w-6 rounded-full flex items-center justify-center text-white text-xs font-bold opacity-0 hover:opacity-100 transition-all duration-200 cursor-pointer"
                    style={{
                      background: 'rgba(99, 102, 241, 0.85)',
                      boxShadow: '0 0 16px rgba(99, 102, 241, 0.6), 0 0 4px rgba(99, 102, 241, 0.3)',
                    }}
...
export default function ApexVisualizerTool() {
  ...
  const [editingStep, setEditingStep] = useState(null);     // { id, label }
  const [editValue, setEditValue] = useState('');
  const [insertingAfter, setInsertingAfter] = useState(null); // step index
  ...
  const customCount = steps.filter(s => s.source === 'custom').length;
  const renamedCount = steps.filter(s => {
    const orig = machineStepsRef.current.find(ms => ms.id === s.id);
    return orig && orig.label !== s.label;
  }).length;
  const removedCount = machineStepsRef.current.filter(
    ms => ms.editable && !steps.find(s => s.id === ms.id)
  ).length;
  const totalMods = customCount + renamedCount + removedCount;
  ...
  useEffect(() => {
    ...
    const timeout = setTimeout(async () => {
      const mermaidStr = stepsToMermaid(steps);
      setMermaidCode(mermaidStr);
      if (!mermaidStr) { setSvgContent(''); return; }
      ...
      try {
        ...
        const { svg } = await mermaid.render(graphId, mermaidStr);
  ...
  // ── 3. SVG rendered → Calculate insertion points ──
  useEffect(() => {
    if (!svgContent || mode !== 'single') {
      setInsertionPoints([]);
      return;
    }
    ...
    requestAnimationFrame(() => {
      ...
      // Get positions sorted top-to-bottom
      const positions = nodeEls.map(node => {
        const rect = node.getBoundingClientRect();
        return {
          cx: rect.left + rect.width / 2 - wrapperRect.left,
          top: rect.top - wrapperRect.top,
          bottom: rect.bottom - wrapperRect.top,
        };
      }).sort((a, b) => a.top - b.top);

      // Match SVG nodes to visible steps (both are in top-down order)
      const visibleSteps = steps.filter(s => !s.hidden);

      const points = [];
      const count = Math.min(positions.length, visibleSteps.length);

      for (let i = 0; i < count - 1; i++) {
        const gap = positions[i + 1].top - positions[i].bottom;
        if (gap < 20) continue; // too cramped

        // Find this step's index in the full array
        const stepId = visibleSteps[i].id;
        const fullIndex = steps.findIndex(s => s.id === stepId);

        points.push({
          x: positions[i].cx,
          y: (positions[i].bottom + positions[i + 1].top) / 2,
          afterStepIndex: fullIndex,
        });
      }

      setInsertionPoints(points);
    });
  }, [svgContent, mode, steps]);

  // ── 4. Node click handler (click-to-rename) ──
  useEffect(() => {
    ...
    function handleNodeClick(e) {
      const nodeEl = e.target.closest('[id^="flowchart-"]') || e.target.closest('.node');
      if (!nodeEl) return;

      // Extract step ID from the element ID (e.g., "flowchart-n2-123" → "n2")
      const idMatch = nodeEl.id?.match(/(n\d+|c\d+)/);
      if (!idMatch) return;
      const nId = idMatch[1];

      const step = steps.find(s => s.id === nId);
      if (!step || !step.editable) return;

      setEditingStep({ id: nId, label: step.label });
      setEditValue(step.label);
    }

    wrapper.addEventListener('click', handleNodeClick);
    return () => wrapper.removeEventListener('click', handleNodeClick);
  ...
  const renderMermaidCompare = useCallback(async (code, graphPrefix, renderId) => {
    ...
    try {
      ...
      return { svg: '', error: err?.message || 'Rendering failed' };
    }
  }, [uniqueId]);
  ...
  useEffect(() => {
    ...
    const timeout = setTimeout(async () => {
      ...
      const result = await renderMermaidCompare(trimmed, 'cmpA', thisRender);
      if (thisRender === renderARef.current) {
        setSvgA(result.svg); setErrorA(result.error); setRenderingA(false);
      }
    }, 400);
    return () => clearTimeout(timeout);
  }, [compareA, mode, renderMermaidCompare]);
  ...
  useEffect(() => {
    ...
    const timeout = setTimeout(async () => {
      ...
      const result = await renderMermaidCompare(trimmed, 'cmpB', thisRender);
      if (thisRender === renderBRef.current) {
        setSvgB(result.svg); setErrorB(result.error); setRenderingB(false);
      }
    }, 400);
    return () => clearTimeout(timeout);
  }, [compareB, mode, renderMermaidCompare]);
  ...
  // ── Mid-flow insertion ──
  function handleInsertAt(afterStepIndex) {
    setInsertingAfter(afterStepIndex);
    setNewStepLabel('');
  }

  function handleConfirmInsert() {
    if (insertingAfter === null || !newStepLabel.trim()) return;
    const newId = `c${++customIdRef.current}`;
    const newStep = {
      id: newId,
    ...
    setSteps(prev => {
      const next = [...prev];
      next.splice(insertingAfter + 1, 0, newStep);
      return next;
    });
    setNewStepLabel('');
  ...
  function handleRenameSubmit() {
    ...
    setSteps(prev => prev.map(s =>
      s.id === editingStep.id ? { ...s, label: editValue.trim() } : s
    ));
    setEditingStep(null);
    setEditValue('');
  }

  // ── Hide (remove from flow) ──
  function handleHideStep(stepId) {
  ...
  function handleResetSteps() {
    setSteps(JSON.parse(JSON.stringify(machineStepsRef.current)));
    customIdRef.current = 0;
  }
  ...
  // ── Insertion modal helpers ──
  const beforeStep = insertingAfter !== null ? steps[insertingAfter] : null;
  const afterStep = insertingAfter !== null
    ? steps.slice(insertingAfter + 1).find(s => !s.hidden)
    : null;
  ...
  return (
    <div className="min-h-screen bg-background text-foreground">
      ...
      {/* ── Insert Step Modal (mid-flow) ── */}
      {insertingAfter !== null && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
          <div className="mx-4 w-full max-w-md rounded-2xl border border-border bg-surface p-6 shadow-2xl">
            <h3 className="text-sm font-semibold text-foreground">Insert Step</h3>
            <p className="mt-1 text-xs text-muted">
              Add a business process step between{' '}
              <span className="font-medium text-accent">&ldquo;{beforeStep?.label || '...'}&rdquo;</span>{' '}
              and{' '}
              <span className="font-medium text-accent">&ldquo;{afterStep?.label || '...'}&rdquo;</span>.
            </p>
            <input
              type="text"
              value={newStepLabel}
              ...
              onKeyDown={(e) => {
                ...
                if (e.key === 'Escape') setInsertingAfter(null);
              }}
              ...
              placeholder="e.g., Manual Approval Step, Send Confirmation Email"
              autoFocus
            />
            <div className="mt-4 flex items-center justify-end gap-2">
              <button
                onClick={() => { setInsertingAfter(null); setNewStepLabel(''); }}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
          <div className="mx-4 w-full max-w-md rounded-2xl border border-border bg-surface p-6 shadow-2xl">
            ...
            <div className="mt-4 flex items-center justify-between">
              <button
                onClick={() => handleHideStep(editingStep.id)}
                className="text-xs font-medium text-red-400 hover:text-red-300 transition-colors"
                title="Remove this node from the flowchart"
              >
                Remove Node
              </button>
              <div className="flex items-center gap-2">
