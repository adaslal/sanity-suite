'use client';
/**
 * ApexVisualizerTool v4 — Business Process Map + Stickiness Features
 *
 * Features:
...
function FlowchartPanel({ svgContent, error, isRendering, containerRef, onLoadSample }) {
  return (
    <div className="flex flex-col rounded-xl border border-border bg-surface overflow-hidden h-full">
      <div className="flex items-center justify-between border-b border-border/50 px-4 py-2.5">
        <div className="flex items-center gap-2">
      ...
      <div className="relative flex-1 overflow-auto" style={{ minHeight: '400px' }}>
        {error ? (
          ...
          <div className="flex flex-col h-full">
            <div
              ref={containerRef}
              className="flex-1 flex justify-center p-6 [&_svg]:max-w-full [&_svg]:h-auto"
...
export default function ApexVisualizerTool() {
  ...
  const renderMermaid = useCallback(async (code, graphPrefix, renderId) => {
    const mermaid_output = parseApexToMermaid(code);
  ...
  useEffect(() => {
    ...
    if (!trimmed) {
      setMermaidCode(''); setSvgContent(''); setStats(null); setFindings([]); setError(null);
      return;
    }
    ...
    const timeout = setTimeout(async () => {
      ...
      const result = await renderMermaid(trimmed, 'viz', thisRender);
